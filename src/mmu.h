...
void mmu_init(mmu *self, uint32_t memsize, FILE *bootrom_ptr, FILE *romptr);
...
uint16_t mmu_read16(mmu *self, uint16_t addr);
void mmu_write16(mmu *self, uint16_t addr, uint16_t val);
